...
- [分步实现](#%E5%88%86%E6%AD%A5%E5%AE%9E%E7%8E%B0)
  - [1 注册命令](#1-%E6%B3%A8%E5%86%8C%E5%91%BD%E4%BB%A4)
    - [step-1: 初始化工程](#step-1-%E5%88%9D%E5%A7%8B%E5%8C%96%E5%B7%A5%E7%A8%8B)
    - [step-2: 创建 `src/bin.js` 文件](#step-2-%E5%88%9B%E5%BB%BA-srcbinjs-%E6%96%87%E4%BB%B6)
    - [step-3: 调整 `package.json`](#step-3-%E8%B0%83%E6%95%B4-packagejson)
    - [step-4: 运行 `npm link`](#step-4-%E8%BF%90%E8%A1%8C-npm-link)
  ...
  - [5. 修改文件](#5-%E4%BF%AE%E6%94%B9%E6%96%87%E4%BB%B6)
    - [创建 `update-files.js` 文件](#%E5%88%9B%E5%BB%BA-update-filesjs-%E6%96%87%E4%BB%B6)
...
我们需要使用 `npm link` 命令将我们的可执行文件(bin)链接到 `{prefix}/bin/{name}`。换句话说，`npm link` 命令可以让我们在任意位置使用命令行直接运行我们的脚本。

#### step-1: 初始化工程

首先初始化一个工程，并在工程中安装 `.editorconfig`、`eslint.js`、`prettier.js` 等插件。详细可以参考 [一个易维护的 webpack 工程](https://git.fzyun.io/frontend/docs/webpack-tutorial)

示例工程： [examples/01](./examples/01)

#### step-2: 创建 `src/bin.js` 文件
...
[commander.js](https://github.com/tj/commander.js/blob/master/Readme_zh-CN.md#%e5%91%bd%e4%bb%a4) 是一个轻巧的nodejs模块，提供了用户命令行输入和参数解析强大功能。`commander.js` 是从 `Ruby`同名项目移植过来的。详细内容可以参考 [commander.js 文档](https://github.com/tj/commander.js/blob/master/Readme_zh-CN.md#%e5%91%bd%e4%bb%a4)
...
const answers = getUserInputs()
console.info(answers)
```

用 `node` 运行 `src/prompts.js` ，查看结果

![](./assets/023-promps.gif)
...
这一步比较简单，我们借助 [cross-spawn](https://www.npmjs.com/package/cross-spawn) 在下载的工程中执行`yarn || npm`脚本。
...
- NPM 仓库服务: https://kb.fzyun.io/projects/rdc/wiki/NPM_%E4%BB%93%E5%BA%93%E6%9C%8D%E5%8A%A1
